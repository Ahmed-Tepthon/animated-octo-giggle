...
def get_my_rank(c,m,k):
   ...
   if text == 'معلوماتي':
      ...
      if None:
         username = ''
         for i in None: username += f"@{i.username} "
      else:
         username = 'مافي يوزر'
...
'''
   ...
   if text.startswith('ايدي') and len(text.split()) > 1 and 'tg://user?id=' in m.text.html:
      try:
         user = int(re.search(r'href="([^"]+)', m.text.html).group(1).split('=')[1])
         return m.reply(f'الايدي ↢ ( `{user}` )')
      except:
         return m.reply(f'{k} خطأ في استخراج الايدي')
