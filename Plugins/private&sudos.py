...
async def private_func(c,m,k):
  ...
  if text == '/start' and not dev_pls(m.from_user.id,m.chat.id):
     await c.send_video(
         chat_id=m.chat.id,
         video='https://files.catbox.moe/whsxgo.mp4',
         caption=f'''
اهلين انا ،{name} 🧚
...
def SudosCommandsFunc(c,m,k,r,channel):
   ...
   if text == 'تعيين فيديو' and dev2_pls(m.from_user.id,m.chat.id):
      r.set(f'{m.chat.id}:setWelcomeVideo:{m.from_user.id}{Dev_Zaid}', 1)
      return m.reply(quote=True, text=f'{k} ارسل الان رابط الفيديو (telegra.ph او catbox.moe)')

   if text == 'حذف فيديو' and dev2_pls(m.from_user.id,m.chat.id):
      r.delete(f'{Dev_Zaid}:WelcomeVideo')
      return m.reply(quote=True, text=f'{k} تم حذف فيديو الترحيب بنجاح')

   if r.get(f'{m.chat.id}:setWelcomeVideo:{m.from_user.id}{Dev_Zaid}') and dev2_pls(m.from_user.id,m.chat.id):
      r.delete(f'{m.chat.id}:setWelcomeVideo:{m.from_user.id}{Dev_Zaid}')
      if text == 'الغاء':
         return m.reply(f'{k} تم الغاء تعيين الفيديو')
      r.set(f'{Dev_Zaid}:WelcomeVideo', text)
      return m.reply(f'{k} تم تعيين فيديو الترحيب بنجاح')
