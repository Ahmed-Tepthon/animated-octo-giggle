...
def global_filter(c,m):
   ...
   text = m.text
   name = r.get(f'{Dev_Zaid}:BotName') if r.get(f'{Dev_Zaid}:BotName') else 'دراك'
   ...
   if text == 'المطور':
     ...
     dev1_id = 8087077168 
     try:
         get1 = c.get_chat(dev1_id)
         bio1 = get1.bio if get1.bio else ""
         name1 = get1.first_name
         markup1 = InlineKeyboardMarkup([[InlineKeyboardButton(name1, user_id=dev1_id)]])
         
         photo1 = None
         try:
             for p in c.get_chat_photos(dev1_id, limit=1):
                 photo1 = p.file_id
         except:
             pass
         
         if photo1:
             c.send_photo(m.chat.id, photo1, caption=bio1, reply_markup=markup1, parse_mode=ParseMode.HTML)
         else:
             c.send_message(m.chat.id, bio1, reply_markup=markup1, parse_mode=ParseMode.HTML)
     except Exception as e:
         c.send_message(m.chat.id, "", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("dev_mido", url="https://t.me/dev_mido")]]), parse_mode=ParseMode.HTML)

     time.sleep(1)

     # المطور الثاني
     dev2_photo_url = "https://files.catbox.moe/m1o0jp.jpg"
     bio2 = "Legend @Dev_Mido , @Tepthon"
     
     try:
         dev2_info = None
         dev2_id = None
         try:
             get2 = c.get_chat("a_s_q")
             dev2_id = get2.id
             dev2_info = get2
         except:
             pass
         
         if dev2_info and dev2_id:
             markup2 = InlineKeyboardMarkup([[InlineKeyboardButton("| 𝙃 𝙼 𝘿 .", user_id=dev2_id)]])
         else:
             markup2 = InlineKeyboardMarkup([[InlineKeyboardButton("| 𝙃 𝙼 𝘿 .", url="https://t.me/a_s_q")]])
         
         c.send_photo(m.chat.id, dev2_photo_url, caption=bio2, reply_markup=markup2, parse_mode=ParseMode.HTML)
     except Exception as e:
         try:
             c.send_photo(m.chat.id, dev2_photo_url, caption="", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("a_s_q", url="https://t.me/a_s_q")]]), parse_mode=ParseMode.HTML)
         except:
             c.send_message(m.chat.id, "", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("a_s_q", url="https://t.me/a_s_q")]]), parse_mode=ParseMode.HTML)
     return
   ...
   if m.data.startswith(f'{m.from_user.id}quMaher'):
      ...
      return m.edit_message_media(
        ...
        reply_markup=InlineKeyboardMarkup (
            [
            [
              InlineKeyboardButton (title,url='t.me/Tepthon')
            ],
            [
