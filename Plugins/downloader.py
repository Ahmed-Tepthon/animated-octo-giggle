...
from yt_dlp import YoutubeDL as YouTube
...
def yt_func(c,m,k,channel):
   ...
   if text.startswith('بحث ') or text.startswith('yt '):
     ...
     ydl_ops = {'cookiefile': 'cookies.txt', 'format': 'bestaudio/best', 'quiet': True}
     ...
     if False:
         ...
         ydl_ops = {'cookiefile': 'cookies.txt', "format": "bestaudio[ext=m4a]",'forceduration':True, "username": "oauth2", "password": ''}
         with yt_dlp.YoutubeDL(ydl_ops) as ydl:
           info = ydl.extract_info(url, download=False)
         ...
         performer="YouTube",reply_markup=rep)
...
async def shazamFunc(c,m):
   ...
   if m.reply_to_message and (m.reply_to_message.audio or m.reply_to_message.voice or m.reply_to_message.video):
     ...
     if not out["matches"]:
       ...
       k = r.get(f'{Dev_Zaid}:botkey')
       channel = r.get(f'{Dev_Zaid}:BotChannel') if r.get(f'{Dev_Zaid}:BotChannel') else 'DarkTepbot'
       url = out["track"]["url"]
       TEXT = f"""
...
def getInfo(c, query):
    ...
    photo = info.get('thumbnail') or "https://telegra.ph/file/49ace69e7c43c0041fb63.jpg"
...
def audio_down(c, query):
    ...
    # ydl_ops = {'cookiefile': 'cookies.txt', "format": "bestaudio/best", 'forceduration': True}
    ydl_ops = {
...
def video_down(c, query):
    ...
    url = f'https://youtu.be/{vid_id}'
    query.edit_message_caption("جاري التحميل ..", reply_markup=rep)    
    ydl_ops = {'cookiefile': 'cookies.txt', "format": "bestvideo+bestaudio/best"}
    with yt_dlp.YoutubeDL(ydl_ops) as ydl:
        info = ydl.extract_info(url, download=False)
